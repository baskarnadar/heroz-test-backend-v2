...
exports.isUserExist = async (req, res, next) => {
 ...
 exports.IsOtpVerified = async (req, res, next) => {
  try {
    ...
    // 2) Match user using username only
    const query = {
      username: mobile,
    };
    ...
    // 3) Compare OTP manually
    if (String(user.userotp || "").trim() !== otpStr) {
    ...
    const upd = await db.collection("tblprtusers").updateOne(
      { _id: user._id },
      update
    );
 ...
 // controllers/commondata/operation/operation.controller.js (or wherever signup is)

 // ✅ FIXED signup (kept your logic, only corrected issues)
// - Fixed missing/incorrect braces in email duplicate check (your code was broken inside if(emailVal))
// - Added support for UserType === "VENDOR-SUBADMIN" (your code checked it but never handled insert)
// - Prevented email duplicate check when schEmail is "" (empty string)
// - Kept password as rawPassword (same as your current code) — do NOT change unless your login expects hash
...
exports.signup = async (req, res, next) => {
  try {
    ...
    if (!UserType) {
      return sendResponse(res, "UserType is required.", true);
    }

    const NowISO = new Date();

    const username = String(req.body?.mobileno ?? "").trim();
    const rawPassword = String(req.body?.password ?? "").trim();
    const emailValRaw = req.body?.schEmail;
    const emailVal = String(emailValRaw ?? "").trim(); // may be ""
    ...
    // ✅ check username duplicate in tblprtusers
    ...
    // ✅ email duplicate checks only when email provided and not empty
    if (emailVal) {
      // ✅ Vendor subadmin OR vendor types stored in tblvendorinfo
      if (UserType === "VENDOR-SUBADMIN" || UserType === "VENDOR-SUBADMIN" || UserType === "ONECLUB") {
        const existingVendorEmail = await db.collection("tblvendorinfo").findOne({
          vdrEmailAddress: { $regex: `^${emailVal}$`, $options: "i" },
        });
      ...
      // ✅ School subadmin stored in tblschoolinfo
      if (UserType === "SCHOOL-SUBADMIN") {
        const existingSchoolEmail = await db.collection("tblschoolinfo").findOne({
          schEmailAddress: { $regex: `^${emailVal}$`, $options: "i" },
        });
    ...
    // ==========================
    if (UserType === "SCHOOL-SUBADMIN") {
      const SchoolIDVal = generateUniqueId();
      const schName = String(req.body?.schName ?? "").trim();
      ...
      const schoolDoc = {
        ...
        schStatus: "ACTIVE",
        IsDataStatus: "1",
        schEmailAddress: emailVal ? emailVal : null,
      ...
      return sendResponse(res, "School inserted successfully.", false, {
    ...
    // ==========================
    // ✅ VENDOR TYPES (MEMBERSHIP / ONECLUB)
    // ==========================
    if (UserType === "VENDOR-SUBADMIN" || UserType === "VENDOR-SUBADMIN" || UserType === "ONECLUB") {
      ...
      const vendorItem = {
        ...
        vendorNo: generateNo(),
        vdrImageName: "logo.png",
        vdrName: req.body?.schName,
        vdrClubName: req.body?.schName,
        vdrEmailAddress: emailVal ? emailVal : null,
        vdrMobileNo1: username,
        IsDataStatus: "1",
        vdrStatus: req.body?.vdrStatus ?? "ACTIVE",
        CreatedDate: NowISO,
      ...
      await createUser({
        ...
        CreatedBy: VendorIDVal,
        ModifyBy: VendorIDVal,
      });

      return sendResponse(res, "vendor inserted successfully.", false, {
        vendorMongoId: result.insertedId,
        VendorID: VendorIDVal,
        username: username,
    ...
    // ==========================
    // ✅ VENDOR-SUBADMIN (if you are using this UserType directly)
    // ==========================
    if (UserType === "VENDOR-SUBADMIN") {
      const VendorIDVal = generateUniqueId();
      ...
      return sendResponse(res, "vendor inserted successfully.", false, {
        ...
        username: username,
        usertype: "VENDOR-SUBADMIN",
        sourceUserType: "VENDOR-SUBADMIN",
      });
    }

    // ✅ unknown UserType should be error (HTTP 400)
    return sendResponse(res, "Invalid UserType.", true, { UserType });
  } catch (error) {
    return sendResponse(res, "Error in signup.", true, {
      error: String(error?.message || error),
    });
  }
};
...
exports.herozagreement = async (req, res, next) => {
 ...
 // controllers/commondata/operation/operation.controller.js (or your current controller file)
 exports.getagree = async (req, res, next) => {
  try {
    const db = await connectToMongoDB();

    // ✅ Default AgreeID = 100 if nothing provided
    let AgreeIDVal = 100;

    const raw = req.body?.AgreeID;

    // Accept: undefined / null / "" / "   " => default 100
    if (raw !== undefined && raw !== null) {
      const s = String(raw).trim();
      if (s !== "") {
        const n = Number(s);
        if (!Number.isFinite(n)) {
          return sendResponse(res, "AgreeID must be a valid number.", true);
        }
        AgreeIDVal = n;
      }
    }

    // ✅ Match both number and string stored values
    const doc = await db.collection("tblherozagree").findOne(
      {
        AgreeID: { $in: [AgreeIDVal, String(AgreeIDVal)] },
      },
      {
        projection: {
          _id: 0,
    ...
    return sendResponse(res, "Heroz Agreement Fetched.", false, doc);
  } catch (error) {
 ...
 exports.genSchReport = async (req, res, next) => {
  try {
    ...
    const fileName =req.body.RequestID+ "SchReport.pdf";
...
async function createTripReportPdfBuffer(payload) {
  ...
  // ✅ Footer assets + also used for TABLE HEADER background image
  ...
  // ✅ Build table header background image data-uri (using bbg.png)
  const tableHeaderBgCss = footerBgB64
    ? `background-image:url("data:image/png;base64,${footerBgB64}"); background-size:cover; background-position:center;`
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* ✅ NEW: table header background image from assets/icons/bbg.png */
    thead tr { ${tableHeaderBgCss} }
    thead th {
      color:#fff;
      text-align:left;
      padding:6px 10px;
      font-size:9px;
      background: transparent; /* important so image shows */
    }
    ...
    .footerBar {
      ...
      ${footerBgB64 ? `background-image:url("data:image/png;base64,${footerBgB64}"); background-size:cover; background-position:center;` : `background:${COLORS.purple};`}
...
<body>
  ...
  <div class="box">
    <div class="grid3">
      ...
      <div>
        ...
        <div class="valueBox">${escapeHtml(
          (gradesArr.length ? gradesArr : ["-"]).map(sanitizeText).join("\n")
        )}</div>
      </div>
      ...
      <div>
        ...
        <div class="valueBox">${escapeHtml(
          (teachersLines.length ? teachersLines : ["-"]).map(sanitizeText).join("\n")
        )}</div>
    ...
    <div class="grid2">
      ...
      <div>
        ...
        <div class="cta">
          ...
          <div class="ctaSub">${escapeHtml(
            CTA_SUBTITLE || "Book your next school trip with heroz"
          )}</div>
        </div>
      </div>
    </div>
  ...
  <div class="pair">
    <div>
      ...
      <div class="outlined">${escapeHtml(
        (fixedOutcomes.length ? fixedOutcomes : ["-"]).join("\n")
      )}</div>
    </div>
    <div>
